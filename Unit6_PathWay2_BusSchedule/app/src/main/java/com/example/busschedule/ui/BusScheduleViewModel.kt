...
class BusScheduleViewModel(private val repository: BusScheduleRepository): ViewModel() {
    ...
    fun getFullSchedule(): Flow<List<BusSchedule>> = repository.getAllStream()

    // Get example bus schedule by stop
    fun getScheduleFor(stopName: String): Flow<List<BusSchedule>> = repository.getForStopNameStream(stopName)
